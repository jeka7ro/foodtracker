...
const check = async () => {
    ...
    // Fetch for Cluj (90296b11)
    const res = await fetch('https://api-eu.syrve.live/api/1/deliveries/by_delivery_date_and_status', {
        method:'POST',
        headers:{'Content-Type':'application/json','Authorization':`Bearer ${token}`},
        body: JSON.stringify({ organizationIds: ['90296b11-9ba9-4279-a69b-1f84e193315e'], deliveryDateFrom: '2026-05-01 00:00:00.000', deliveryDateTo: '2026-05-02 00:00:00.000', statuses:['Closed'] })
    })
    ...
    console.log(`Found ${allItemNames.size} unique items in Cluj`)
    
    const smashLike = []
    const isSmash = name => {
        const n = name.toLowerCase()
        return n.includes('burger') || n.includes('smash') || n.includes('cartof') || n.includes('crispy') || n.includes('strips') || n.includes('nuggets')
    }
    ...
    console.log("--- Smash-like items in Cluj ---")
