...
const check = async () => {
    ...
    const bad = sales.filter(s => {
        ...
        return s.items.some(it => it.product_name && it.product_name.includes('Big in Japan'))
    })
    
    console.log(`Found ${bad.length} sales with Big in Japan for Ikura.`)
    for (const s of bad.slice(0, 2)) {
        console.log(`Order ID: ${s.order_id}`)
        console.log(`Items: ${JSON.stringify(s.items)}`)
        
        // Find which restaurant this is
